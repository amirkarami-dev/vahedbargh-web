...
import { GitBranch, FolderOpen, AlertTriangle } from "lucide-react";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { title: "فرآیند بازرسی" };
...
export default async function InspectionProcessPage() {
  ...
  const { data: processes, error } = await supabase
    ...
    `)
    .eq("is_delete", false)
    .not("inspection_status", "eq", 2)
    .not("inspection_status", "eq", 99)
    ...
    <div className="p-6 md:p-8" dir="rtl">
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">فرآیند بازرسی</h1>
            <p className="text-sm text-[var(--text-muted)] mt-0.5">
              مدیریت فرآیندهای بازرسی فعال
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-2xl overflow-hidden">
          {!processes || processes.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-[var(--text-muted)]">فرآیند فعالی یافت نشد</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border-primary)] bg-[var(--bg-secondary)]">
                    {["ردیف", "عنوان پرونده", "مالک", "مهندس ناظر", "وضعیت", "تاریخ"].map((h) => (
