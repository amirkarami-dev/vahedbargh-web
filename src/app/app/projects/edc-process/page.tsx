...
export const metadata: Metadata = { title: "فرآیندهای EDC" };
...
export default async function EdcProcessPage() {
  const roles = await getUserRoles();
  if (!hasRole(roles, "ElectAdmin")) redirect("/app");
    ...
    <div className="p-6 md:p-8" dir="rtl">
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">فرآیندهای EDC</h1>
            <p className="text-sm text-[var(--text-muted)] mt-0.5">
              فرآیندهای بازرسی مربوط به توزیع برق
            </p>
          </div>
        </div>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border-primary)] bg-[var(--bg-secondary)]">
                    {["ردیف", "عنوان پرونده", "مالک", "وضعیت", "تأیید", "تاریخ"].map((h) => (
