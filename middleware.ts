...
// Paths that do NOT require authentication even under /admin/* or otherwise
const AUTH_EXCLUDED = [
  /^\/api\/payment(\/|$)/,
  /^\/ep(\/|$)/,
  /^\/project-info(\/|$)/,
...
export async function middleware(request: NextRequest) {
  ...
  // Pass excluded paths straight through without any auth work
  if (AUTH_EXCLUDED.some((re) => re.test(pathname))) {
...
export const config = {
  matcher: [
     ...
     * Auth logic inside the function body further limits actual work to /admin/*.
     */
